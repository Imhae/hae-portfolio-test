...
// --- Hero parallax on the device mockups ----------------------------------
const parallaxTarget = document.querySelector('[data-parallax] .cs-hero__split');
...
// --- Image reveal (high-fidelity showcase) -------------------------------
const revealImages = document.querySelectorAll('.cs-reveal-image');
...
revealImages.forEach((el) => {
  ...
  inView(
    el,
    () => {
      animate(el, { clipPath: ['inset(0 0 100% 0)', 'inset(0 0 0% 0)'] }, { duration: 1, ease: [0.16, 1, 0.3, 1] });
    },
    { amount: 0.3 }
  );
});
